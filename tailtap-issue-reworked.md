...
When enabled, a node replicates outbound encrypted WireGuard packets—never plaintext—to a standard VXLAN collector.
...
The mirrored data unlocks structured observability over WireGuard tunnels:
...
A first pass implementation of "TailTap" is located [here](https://github.com/mlbright/tailscale/tree/tailtap) and has been lightly tested.
It will need to be cleaned up before merging.
...
To be perfectly honest, nothing breaks if we never build or ship this feature. Tailscale continues to work as it always has, and users can still debug connectivity issues with `tcpdump` on both endpoints.

Nevertheless, it can be useful to export structured metadata about WireGuard packets to external collectors without running packet captures on every node, especially in large deployments or large enterprises.

Tailtap enables users to:

- route WireGuard health metadata to centralized collectors (Splunk, Elastic, Datadog, cPacket) alongside their existing network telemetry for fleet-wide dashboards.
